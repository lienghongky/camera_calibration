...
for path in glob.glob('data/checkerboards/*.png'):
...
np.savez('output/calib.npz', mtx=mtx, dist=dist, rvecs=rvecs, tvecs=tvecs)
...
img = cv2.imread('data/checkerboards/0002.png')
h, w = img.shape[:2]
...
# save image 
cv2.imwrite('output/undistorted.png', undistortedImg)
